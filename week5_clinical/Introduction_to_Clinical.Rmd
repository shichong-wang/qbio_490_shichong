...
clinic <- clin_data$clinical_patient_brca
...
```{r}
write.csv(clinic, "/home1/shichong/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/shichong/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
y <- clinic$last_contact_days_to

plot(x, y, 
     main = "Scatter Plot of Age at Diagnosis vs Last Contacted Days To", # Plot title
     xlab = "Age at Diagnosis", # X-axis label
     ylab = "Last Contacted Days To", # Y-axis label
...
```{r Exercise 4.2}
# create plot here
clinic_clean <- clinic[3:1099, ]
clinic_clean <- clinic_clean[!is.na(clinic$age_at_diagnosis) & !is.na(clinic$vital_status), ]

# ensure that age_at_diagnosis is numeric
clinic_clean$age_at_diagnosis <- as.numeric(as.character(clinic_clean$age_at_diagnosis))

# ensure that vital_status is a categorical variable
clinic_clean$vital_status <- as.factor(clinic_clean$vital_status)

# create the box plot - vital_status vs age_at_diagnosis
boxplot(clinic_clean$age_at_diagnosis ~ clinic_clean$vital_status,
        main = "Box Plot of Age at Diagnosis by Vital Status", # plot title
        xlab = "Vital Status", 
...
cleaned_clinic$survival_time <- ifelse(cleaned_clinic$death_days_to == "[Not Applicable]",
                                       as.numeric(cleaned_clinic$last_contact_days_to),
                                       as.numeric(cleaned_clinic$death_days_to))
```
...
p-value = 0.2 (>0.05), not statistically significant; there is not statistical correlation between age and breast cancer survival.
